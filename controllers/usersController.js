...
exports.updateSingleUser = async (request, response, next) => {
  try {
    ...
    const updatedUser = await UsersModel.findByIdAndUpdate(
      request.user.id,
      userData,
      { runValidators: true, new: true, select: "name email photo id" }
    );
...
exports.updateMyData = async (request, response, next) => {
  try {
    const userData = {
      ...
      "metadata.facebookurl": request.body.facebookurl
        ? request.body.facebookurl
        : null,
      "metadata.twitterurl": request.body.twitterurl
        ? request.body.twitterurl
        : null,
      "metadata.linkedinurl": request.body.linkedinurl
        ? request.body.linkedinurl
        : null,
      "metadata.pinteresturl": request.body.pinteresturl
        ? request.body.pinteresturl
        : null,
    };
    ...
    if (request.user.provider === "manual") {
      ...
      updatedUser = await SocialUsersModel.findByIdAndUpdate(
        request.user.id,
        userData,
        { runValidators: true, new: true }
      );
