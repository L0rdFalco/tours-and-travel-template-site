...
exports.getManageDestinationsPage = async (request, response, next) => {
  try {
    const destinationDocs = await destinationModel
      .find()
      .select("_id name price location imageCover slug");
...
exports.getManageHotelsPage = async (request, response, next) => {
  const hotelDocs = await hotelsModel
    .find()
    .select("_id name price city summary imageCover");
  try {
...
exports.getManageRestaurantsPage = async (request, response, next) => {
  try {
    const restaurantDocs = await restaurantModel
      .find()
      .select("_id name price city summary imageCover");
...
exports.getManageToursPage = async (request, response, next) => {
  try {
    const tourDocs = await toursModel
      .find()
      .select(
        "_id name summary startLocation price imageCover duration maxGroupSize"
      );
...
exports.getMessagesPage = async (request, response, next) => {
  try {
    ...
    if (request.user.role === "user") {
      messageDocs = await messagesModel
        .find()
        .select("_id subject message state updatedAt");
      ...
      const messageDocs = await messagesModel
        .find()
        .select("_id subject message state updatedAt");
      response.status(200).render("messages", {
        payload: messageDocs,
...
exports.getAllToursGridSidebarPage = async (request, response, next) => {
  try {
    const alltours = await toursModel
      .find()
      .select(
        "name duration difficulty slug ratingsAverage ratingsQuantity summary price startLocation difficulty"
      );
...
exports.getSingleTourDetailPage = async (request, response, next) => {
  try {
    let singleTour = await toursModel
